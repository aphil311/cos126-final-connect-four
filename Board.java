...
public class Board {
    ...
    public boolean isFull(int j) {
        if (j >= board[0].length || j < 0)
            throw new IllegalArgumentException("isFull parameter not within board size");
        return height[j] == -1;
    }
    ...
    public void insert(int c, int p) {
        if (c >= board[0].length || c < 0)
            throw new IllegalArgumentException("insert parameter not within board size");
        if (this.isFull(c))
            throw new IllegalArgumentException("cannot insert ... column is already full");
        board[height[c]][c] = p;
        --height[c];
    }
