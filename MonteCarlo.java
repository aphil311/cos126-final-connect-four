...
public class MonteCarlo extends Player {
     ...
     */
    public MonteCarlo(int val, Board board, int opp, int iterations) {
        super(val, board);
        this.opp = opp;
    ...
    public void move() {
        ...
        for (int timer = 0; timer < iterations; timer++) {
            ...
            while (!node.isLeaf()) {
                if (!node.state
                        .isFull()) // if board is full returnBestUCT gives error (no children)
            ...
            for (int i = 0; i < state.getCols(); i++) {     // iterate through each possible move
                if (!state.isFull(i)) {     // prevent from inserting in a column that is full
                    Board newBoard = new Board(state.getBoard(),
                                               state.getHeights());   // defensive copy

                    newBoard.insert(i, val);
                    // this algorithm requires the assumption of strong play from its opponent
                    newBoard.insert(educatedRandom(newBoard.getBoard(), newBoard.getHeights()),
                                    opp);
